...
public class ArrayDeque<T> extends AbstractDeque<T> {
    ...
    @SuppressWarnings("unchecked")
    private void resize(int newCapacity, int oldCapacity) { // originally only parameter is capacity
        T[] newData = (T[]) new Object[newCapacity];
        int i = increment(front, oldCapacity); //originally front,size
        for (int newIndex = 0; newIndex < size; newIndex += 1) {
            newData[newIndex] = data[i];
            i = increment(i, oldCapacity);
        }
